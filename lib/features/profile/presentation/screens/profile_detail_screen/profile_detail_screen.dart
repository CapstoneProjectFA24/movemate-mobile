...
class ProfileDetailScreen extends HookConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final userFuture = SharedPreferencesUtils.getInstance('user_token');

    final useFetchResult = useFetchObject<ProfileEntity>(
      function: (context) async {
        final user = await userFuture;
        final userId = user?.id;
        return profileController.getProfileInforById(userId!, context);
      },
      context: context,
    );
    final profileUser = useFetchResult.data;
    print("object ${profileUser?.email.toString()}");
